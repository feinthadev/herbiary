...
public class OwlEntity extends TameableShoulderEntity implements Angerable, GeoEntity, Flutterer {
    ...
    public boolean isAnotherOwlOnNest() {
        return this.method_48926().getEntitiesByClass(OwlEntity.class, Box.of(Vec3d.ofCenter(this.nest), 1, 18, 1), entity -> entity.nest != null).size() > 0;
    }

    public boolean isAnotherOwlNearNestPos(Vec3d pos, int horizDist) {
        return this.method_48926().getEntitiesByClass(OwlEntity.class, Box.of(pos, horizDist, 18, horizDist), entity -> entity.nest != null).size() > 0;
    }
    ...
    private static class OwlEntityAIGoal extends Goal {
        ...
        private BlockPos locateNestPos() {
            ...
            do {
                ...
                BlockState blockState = this.mob.method_48926().getBlockState(mutable2.set(blockPos2, Direction.DOWN));
                BlockState _s = this.mob.method_48926().getBlockState(blockPos2);
                if (_s.isOf(ModItems.BIRD_NEST_BLOCK.getLeft())) {
                    NestBlockEntity _e = ((NestBlockEntity) this.mob.method_48926().getBlockEntity(blockPos2));
                ...
                bl=true;
            } while(!bl || !this.mob.method_48926().isAir(blockPos2) || !this.mob.method_48926().isAir(mutable.set(blockPos2, Direction.UP)));
        ...
        public void tick() {
            ...
            if (e.nest == null || e.nest.equals(BlockPos.ORIGIN)) {
                ...
                if (--ticks_since_last_nav <= 0) {
                    ...
                    if (e.method_48926().getBlockState(e.getBlockPos().down()).isIn(Herbiary.NEST_COLLECTABLES) && wants_collectable) {
                    ...
                    if (this.isFindingNestSpot && e.method_48926().getBlockState(e.getBlockPos().down()).isIn(Herbiary.BIRD_NEST_PLACEABLE)) {
                        e.nest = e.getBlockPos();
                        // set nest block and set owner
                        e.method_48926().setBlockState(e.getBlockPos(), ModItems.BIRD_NEST_BLOCK.getLeft().getDefaultState());
                        NestBlockEntity nestBlockEntity = ((NestBlockEntity)e.method_48926().getBlockEntity(e.getBlockPos()));
            ...
            if (e.nest != null){
                BlockState NEST_STATE = e.method_48926().getBlockState(e.nest);
                if (e.method_48926().isNight() || e.method_48926().isRaining()) {
                    Vec3d _d = Vec3d.ofCenter(e.nest);
                    BlockState d = this.mob.method_48926().getBlockState(this.mob.getBlockPos().down());
