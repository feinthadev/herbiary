...
public class TentEntity extends Entity implements GeoEntity, Attackable {
    ...
    public TentEntity(EntityType<? extends Entity> type, World world) {
        super( type, world);
        this.method_48926().spawnEntity(new TentStorageEntity(ModEntityTypes.TENT_STORAGE_ENTITY_ENTITY_TYPE, world, this));
    }
        ...
        if (player instanceof ServerPlayerEntity entity) {
            if (this.dataTracker.get(HAS_BED)) {
                if (entity.isSneaking()) {
                    ...
                    this.method_48926().spawnEntity(new ItemEntity(method_48926(), itemPosToSpawn.getX(), itemPosToSpawn.getY(), itemPosToSpawn.getZ(), ModItems.SLEEPING_BAG.getDefaultStack()));
                    this.dataTracker.set(HAS_BED, false);
                    return ActionResult.CONSUME;
                } else if (entity.method_48926().isNight()){
    ...
    public boolean damage(DamageSource source, float amount) {
        if (source.isOf(DamageTypes.PLAYER_ATTACK)) {
            ...
            if (this.hasSleepingBag()) {
                this.method_48926().spawnEntity(new ItemEntity(method_48926(), itemPosToSpawn.getX(), itemPosToSpawn.getY() - 0.5f, itemPosToSpawn.getZ(), ModItems.SLEEPING_BAG.getDefaultStack()));
            }
            this.method_48926().spawnEntity(new ItemEntity(method_48926(), itemPosToSpawn.getX(), itemPosToSpawn.getY(), itemPosToSpawn.getZ(), ModItems.TENT_SPAWNER.getDefaultStack()));
            this.kill();
        }
        return false;
    }
