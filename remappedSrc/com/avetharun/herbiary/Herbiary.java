...
public class Herbiary implements ModInitializer {
    ...
    public static void sendPlayerBreakables(ServerPlayerEntity player) {
        ...
        p.allow_vanilla_breaking = player.method_48926().getGameRules().getBoolean(ALLOW_VANILLA_BLOCK_BREAKING);
        p.allow_herb_placement = player.method_48926().getGameRules().getBoolean(ALLOW_HERB_PLACEMENTS);
        PacketByteBuf out = PacketByteBufs.create();
        p.write(out);
        ServerPlayNetworking.send(player, Herbiary.SYNC_BREAKABLES_PACKET_ID, out);
    }
    ...
    public void onInitialize() {
        ...
        EntitySleepEvents.MODIFY_SLEEPING_DIRECTION.register((entity, sleepingPos, sleepingDirection) -> {
            var ents = alib.getEntitiesOfTypeInRange(entity.method_48926(), sleepingPos, 1.25f, ModEntityTypes.TENT_ENTITY_TYPE);
            if (ents.size() > 0) {
        ...
        ServerPlayNetworking.registerGlobalReceiver(IGNITER_IGNITE_PACKET_ID, (server, player, handler, buf, responseSender) -> {
            ...
            if (p.succeeded && player.getBlockPos().getSquaredDistance(p.pos.toCenterPos()) < 9*9) {
                FlintIgniter.HandlePlacement(p.pos, player.method_48926(), player, p.hand, p.result, p.stack);
            }
